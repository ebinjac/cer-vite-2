...
interface CertificateDeletionStatusProps {
...
export function CertificateDeletionStatus({
  results,
  isCompleted,
  inProgress,
  currentIndex,
  totalCount
}: CertificateDeletionStatusProps) {
  ...
  const getStatusSummary = () => {
    if (inProgress) {
      return `Deleting certificates... (${currentIndex}/${totalCount})`
    }
    
    if (isCompleted) {
      if (successCount === totalCount) {
        return 'All certificates deleted successfully'
      } else if (successCount > 0 && failureCount > 0) {
        return `Completed with issues: ${successCount} succeeded, ${failureCount} failed`
      } else if (failureCount === totalCount) {
        return 'Failed to delete certificates'
      }
    }
    
    return 'Ready to delete certificates'
  }
